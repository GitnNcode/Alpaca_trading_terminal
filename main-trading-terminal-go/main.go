...
type termApp struct {
	...
	chartEMATV     *tview.TextView
	chartZoomTV    *tview.TextView
	chartStatsTV   *tview.TextView
	chartCanvasV   *chartCanvas
	chartRangeIdx  int
	chartTFIdx     int
	chartEMAIdx    int // index into emaPeriods (0 = OFF)
	chartAutoOpen  bool
	// Monotonic counter incremented on every loadChart call. Stale responses
	// (network slower than user clicks) compare and bail out, so the chart
	// always reflects the latest user selection — never an out-of-order result.
	chartLoadGen atomic.Int64
	...
	chartEMAHitRanges   [][2]int
	chartZoomHitRanges  [][2]int
}
...
func (a *termApp) globalKeys(event *tcell.EventKey) *tcell.EventKey {
	...
	// Tab navigation uses '<' and '>' (handled below in the rune switch). The
	// arrow keys are reserved for: caret movement in text fields, row navigation
	// in tables, and chart pan/scroll. They never switch tabs anymore.
	...
	case *tview.InputField:
		...
		return event
	case *tview.DropDown:
		// Only TAB-NAVIGATION shortcuts (1-5, <, >) and F5 may bypass the
		// dropdown. Q/R must NOT — pressing 'q' on the trade form's ACTION/TYPE
		// dropdown should be inert, not quit the app. Letters fall through to
		// the dropdown's own (no-op) type-to-search.
		r := event.Rune()
		isTabShortcut := (r >= '1' && r <= '5') || r == '<' || r == '>'
		if !isTabShortcut && event.Key() != tcell.KeyF5 {
			return event
		}
		// fall through to global tab-switch handlers below
	case *tview.Button:
		// Same protection for form buttons (PLACE ORDER / CLEAR). Users tab
		// to them frequently and a stray 'q' must not exit the app.
		r := event.Rune()
		isTabShortcut := (r >= '1' && r <= '5') || r == '<' || r == '>'
		if !isTabShortcut && event.Key() != tcell.KeyF5 {
			return event
	...
	case tcell.KeyEscape:
		// On the chart canvas, Escape returns to the symbol input rather than
		// quitting — matches user expectation. Q / Ctrl-C still quit.
		if a.tapp.GetFocus() == a.chartCanvasV {
...
func (a *termApp) refreshStatus() {
	...
	hint := "[#555555][Q[]UIT  [R[]/F5 REFRESH  [1[][2[][3[][4[][5[] TABS  ·  [<[][>[] CYCLE TABS  ·  MOUSE OK[-]"
	switch a.activeTab {
	case tabOrders:
		hint = "[#555555][Q[]UIT  [R[]/F5 REFRESH  [X[]/DEL OR RIGHT-CLICK CANCEL  ·  [<[][>[] TABS[-]"
	case tabPositions:
		hint = "[#555555][Q[]UIT  [R[]/F5 REFRESH  DOUBLE-CLICK POSITION → SELL  ·  [<[][>[] TABS[-]"
	case tabChart:
		hint = "[#555555][Q[]UIT  RANGE [D[][W[][M[][T[]YD [Y[][F[]IVE MA[X[]  TF [{[][}[]  [E[]MA  [Z[]OOM  X-SCROLL ←→ , .  Y-PAN ↑↓  [0[]=RESET-Y  [<[][>[] TABS[-]"
	}
