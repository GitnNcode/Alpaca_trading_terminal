...
// Credentials are persisted under the SAME path the canonical alpaca-tui build
// uses, so swapping binaries does not require re-entering keys.
//
...
// runSetup shows a first-run credential form using the Bloomberg-style palette
// shared with the canonical build.
func runSetup() Credentials {
	...
	form.
		...
		SetTitle(" [#FF6600::b]BACKTEST TUI — FIRST-TIME SETUP[-] ").
	...
	hintTV := tview.NewTextView().SetDynamicColors(true).
		SetText("  [#555555]Shared with alpaca-tui. Saved to your OS config directory. Run with --reset to re-enter.[-]")
